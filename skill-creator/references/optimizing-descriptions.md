...
With 20 queries at 3 runs each, that's 60 invocations. You'll want to script this. Here's the general structure — adapt the invocation and detection logic to your agent client:
...
# This example uses Pi's JSON output to check for read tool calls targeting the skill.
# Adapt this function with detection logic for your agent client.
# Should return 0 (success) if the skill was invoked, 1 otherwise.
check_triggered() {
  local query="$1"
  pi -p "$query" --mode json --no-session --skill /path/to/skill 2>/dev/null \
    | jq -e --arg skill "$SKILL_NAME" \
      'any(.. | objects | select(.type? == "toolCall" and .name? == "read") | .arguments.path // ""; contains($skill))' \
      > /dev/null 2>&1
}
